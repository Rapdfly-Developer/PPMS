...
function NavLink({ item, active, locked = false, onClick }: { item: NavItem; active: boolean; locked?: boolean; onClick?: () => void }) {
  ...
  return (
    <>
      ...
      <div
        ...
        onClick={close}
        className={clsx(
          "fixed inset-0 z-40 bg-black/50 lg:hidden transition-opacity duration-[250ms]",
      ...
      <aside
        className={clsx(
          ...
          "lg:sticky lg:top-0 lg:h-screen lg:z-auto lg:shrink-0",
      ...
      >
        ...
        <div aria-hidden className="pointer-events-none absolute -top-24 -left-24 size-72 rounded-full -z-10"
          style={{ background: "radial-gradient(circle, rgba(28,147,136,0.4), transparent 68%)" }} />
        <div aria-hidden className="pointer-events-none absolute bottom-16 -right-24 size-64 rounded-full -z-10"
        ...
        <div className="flex items-center gap-3 px-5 pt-7 pb-6">
          ...
          <button
            ...
            className="lg:hidden shrink-0 p-1.5 rounded-lg text-[#7FAAA3] hover:text-[#E4F5F2] hover:bg-white/[0.08] transition-colors"
          >
            <X size={17} />
          </button>
        </div>
...
export function MobileNav({ role, permissions, licenseActive = true }: { role: Role; permissions: string[]; licenseActive?: boolean }) {
  ...
  return (
    <nav
      className="lg:hidden fixed bottom-0 left-0 right-0 z-30 flex border-t border-white/[0.08]"
      style={{
