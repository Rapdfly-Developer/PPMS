...
export async function createDoctor(
  ...
  const shortCode = (formData.get("shortCode") as string)?.trim().toUpperCase() || null;
  ...
  if (shortCode && !/^[A-Z0-9]{2,8}$/.test(shortCode))
  ...
  if (shortCode) {
    const existingCode = await prisma.doctor.findUnique({ where: { shortCode } });
    if (existingCode) return { error: `Short code "${shortCode}" is already in use.` };
  }
  ...
  await prisma.doctor.create({
    data: { userId: user.id, name, shortCode: shortCode || null, specialty, contact },
  });
