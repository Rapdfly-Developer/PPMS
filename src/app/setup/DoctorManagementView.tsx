...
function StatusChip({ active, onToggle }: { active: boolean; onToggle: () => void }) {
  return (
    <button
      type="button"
      onClick={onToggle}
...
export function DoctorManagementView({ onAddDoctor }: { onAddDoctor: () => void }) {
  ...
  const load = () => {
    fetch("/api/setup/doctors")
      .then((r) => r.json())
      .then((d) => { if (Array.isArray(d)) setDoctors(d); setLoading(false); })
  ...
  const saveEdit = async () => {
    if (!editing) return;
    setSaving(true);
    await fetch(`/api/setup/doctors/${editing.id}`, {
  ...
  const filtered = useMemo(() => {
    ...
    if (sort === "status")    sorted.sort((a, b) => Number(b.active) - Number(a.active));
    if (sort === "license")   sorted.sort((a, b) => Number(b.license.active) - Number(a.license.active));
    return sorted;
  }, [doctors, q, statusFilter, licFilter, sort]);
    ...
    <motion.div variants={stagger} initial="hidden" animate="show" className="max-w-[1200px] flex flex-col gap-5">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-white/80 mt-1.5 max-w-md">
              Manage doctors, monitor account activity, licenses and login access.
            </p>
          </div>
          <button
            onClick={onAddDoctor}
      ...
      >
        ...
        <select value={sort} onChange={(e) => setSort(e.target.value as any)} className={selectCls}>
          ...
          <option value="license">Licensed first</option>
        </select>
      ...
      {loading ? (
        ...
        <motion.div variants={rise} className="rounded-3xl bg-white border border-slate-200 py-16 flex flex-col items-center text-center"
          ...
          <p className="text-sm text-slate-400 mt-1 max-w-xs">
            {doctors.length === 0 ? "Create your first doctor account to get started." : "No doctors match the current filters."}
          </p>
          {doctors.length === 0 && (
            <button
              onClick={onAddDoctor}
            ...
            >
              ...
              <div className="relative flex items-start gap-3.5">
                <motion.div
                  ...
                  className="w-13 h-13 rounded-2xl flex items-center justify-center text-white font-bold text-base shrink-0"
                  style={{ width: 52, height: 52, background: avatarGradient(d.name) }}
                >
                  {initials(d.name)}
                </motion.div>
              ...
              <div className="relative flex items-center gap-1">
                ...
                <IconBtn title="Edit doctor" tone="blue" onClick={() => startEdit(d)}>
                  <Pencil size={14} />
                </IconBtn>
          ...
          {pageItems.map((d) => (
            ...
            >
              <div
                ...
                style={{ width: 42, height: 42, background: avatarGradient(d.name) }}
              >
                {initials(d.name)}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-slate-900 truncate">{d.name}</p>
                <p className="text-xs text-slate-400 font-mono truncate">@{d.username}{d.specialty ? ` · ${d.specialty}` : ""}</p>
              </div>
              <StatusChip active={d.active} onToggle={() => toggleActive(d)} />
              <LicenseChip lic={d.license} />
              {d.license.isTrial && (
      ...
      {!loading && filtered.length > 0 && pageCount > 1 && (
        <motion.div variants={rise} className="flex flex-wrap items-center justify-between gap-3 px-1">
          <p className="text-xs text-slate-400">
            ...
            <span className="font-semibold text-slate-600">{filtered.length} Doctors</span>
          </p>
          ...
          <Modal onClose={() => setEditing(null)}>
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-base font-bold text-slate-900">Edit Doctor</p>
                  <p className="text-xs text-slate-400">@{editing.username}</p>
                </div>
              </div>
