...
import {
  ...
  Phone, MapPin, KeyRound, Users, Activity, AlertTriangle, Loader2,
} from "lucide-react";
...
export function HospitalManagementView({ onAddHospital }: { onAddHospital: () => void }) {
  ...
  const confirmDelete = async () => {
    ...
    setDeleteError("");
    const res = await fetch(`/api/setup/hospitals/${deleting.id}`, { method: "DELETE" });
          ...
          <Modal onClose={() => { setDeleting(null); setDeleteError(""); }}>
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-slate-400 mt-1.5 max-w-xs">
                All doctor links and staff accounts will be removed. This cannot be undone.
              </p>
              {deleteError && (
