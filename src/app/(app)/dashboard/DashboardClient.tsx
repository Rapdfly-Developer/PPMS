...
function PatientAvatar({ name, size = "md" }: { name: string; size?: "sm" | "md" }) {
  ...
  return (
    <div className={clsx("shrink-0 rounded-full flex items-center justify-center font-bold text-white", sz, color)}>
      {initials}
    </div>
  );
}
...
/* ── Appointments bar chart (inline SVG) ───────────────────────────────── */
    ...
    <svg viewBox={`0 0 ${totalW} ${chartH + 18}`} className="w-full" preserveAspectRatio="none">
      {data.map(({ h, scheduled, completed }, i) => {
        ...
        return (
          <g key={h}>
            {schedH > 0 && (
              <rect x={x} y={chartH - totalH} width={barW} height={schedH} fill="#93c5fd" rx={2} />
            )}
            {complH > 0 && (
              <rect x={x} y={chartH - complH} width={barW} height={complH} fill="#10b981" rx={2} />
            )}
            {totalH === 0 && (
              <rect x={x} y={chartH - 3} width={barW} height={3} fill="#e5e7eb" rx={1} />
            )}
            <text x={x + barW / 2} y={chartH + 12} textAnchor="middle" fontSize={7} fill="#9ca3af">
              {label}
            </text>
          </g>
        );
...
function PatientDonut({ segments, total }: {
  ...
  return (
    <svg width={84} height={84} viewBox="0 0 84 84" className="shrink-0">
      ...
      <text x={cx} y={cy + 8} textAnchor="middle" fontSize={6} fill="#9ca3af">
        Total Patients
      </text>
    </svg>
  );
...
function TableApptRow({ appt, scope, serial, canManageQueue, canViewPatient }: {
  appt: Appt; scope: "DOCTOR" | "HOSPITAL"; serial: number; canManageQueue: boolean; canViewPatient: boolean;
}) {
  ...
  return (
    <tr className="border-b border-gray-50 hover:bg-blue-50/40 transition-colors">
        ...
        <div className="flex items-center gap-1.5">
          {scope === "DOCTOR" ? (
            <>
              ...
              <span className="text-[12px] text-[var(--color-ink-600)] truncate max-w-[120px]">
                {appt.hospital?.name ?? "—"}
              </span>
            </>
          ) : (
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-1.5">
          {isInConsultation || appt.status === "CONFIRMED" ? (
            <Link
          ...
          {appt.status === "CONFIRMED" && canManageQueue && (
            <button
              disabled={undoing}
              title="Move back to appointment time"
...
/* ── Partial dispense row (kept for full functionality) ─────────────────── */
function PartialDispenseRow({ appt: a, scope, serial, canDispense, canViewPatient }: {
  appt: Appt; scope: "DOCTOR" | "HOSPITAL"; serial: number; canDispense: boolean; canViewPatient: boolean;
}) {
  const [undoing, startUndo] = useTransition();
  return (
    <tr className="border-b border-orange-100 bg-orange-50/50 hover:bg-orange-50 transition-colors">
      ...
      <td className="px-4 py-3 hidden xl:table-cell">
        {scope === "DOCTOR" && a.hospital && (
          <span className="text-[12px] text-[var(--color-ink-600)]">{a.hospital.name}</span>
        )}
        {scope === "HOSPITAL" && a.doctor && (
          <span className="text-[12px] text-[var(--color-ink-600)]">Dr. {a.doctor.name}</span>
        )}
      </td>
      <td className="px-4 py-3">
...
export function DashboardClient({
  ...
  const kpi = useMemo(() => {
    ...
    const yCount       = yesterdayCount;
    const totalPct     = yCount > 0 ? Math.round(((total - yCount) / yCount) * 100) : null;
    return { total, waiting, inConsult, completed, noShow, followUp, newPts, totalPct };
  }, [filteredAppts, yesterdayCount]);

  /* Patient flow derived */
  ...
  /* All rows for table: active (not terminal) + dispensed, then partial, sorted by time */
  const tableRows = useMemo(() => {
    const active   = filteredAppts.filter((a) => !["PARTIAL_DISPENSE"].includes(a.status));
    const partial  = filteredAppts.filter((a) => a.status === "PARTIAL_DISPENSE");
  ...
  return (
    <div className="fade-in flex flex-col gap-5">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {can("appointments.view") && filterOptions.length > 0 && (
              <div className="bg-white rounded-xl border border-[var(--color-border)] p-3 min-w-[160px] shadow-sm">
                <p className="text-[10px] font-semibold text-[var(--color-ink-400)] uppercase tracking-wide mb-1">{filterLabel}</p>
            ...
            <div className="bg-white rounded-xl border border-[var(--color-border)] p-3 shadow-sm text-center min-w-[120px]">
              <p className="text-[10px] font-semibold text-[var(--color-ink-400)] uppercase tracking-wide">{filterOptions.length} Hospital{filterOptions.length !== 1 ? "s" : ""}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        />
        <KpiCard
          icon={<Stethoscope size={18} className="text-[var(--color-primary-600)]" />}
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-[var(--color-border)] bg-[var(--color-surface-sunken)]">
                      <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-[var(--color-ink-400)]">Time</th>
                      <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-[var(--color-ink-400)]">Patient</th>
                      <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-[var(--color-ink-400)] hidden md:table-cell">MRN</th>
                      <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-[var(--color-ink-400)] hidden lg:table-cell">Type</th>
                      <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-[var(--color-ink-400)] hidden xl:table-cell">
                        {scope === "DOCTOR" ? "Hospital" : "Doctor"}
                      </th>
                      <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-[var(--color-ink-400)]">Status</th>
                      <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-[var(--color-ink-400)]">Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {tableRows.partial.map((a, idx) => (
                      <PartialDispenseRow key={a.id} appt={a} scope={scope} serial={idx + 1}
                        canDispense={can("opd.dispense")} canViewPatient={can("patients.view")} />
                    ))}
                    {tableRows.active.map((a, idx) => (
                      <TableApptRow key={a.id} appt={a} scope={scope} serial={idx + 1}
        ...
        <div className="xl:w-80 2xl:w-96 shrink-0 flex flex-col gap-4">
          ...
          <div className="bg-white rounded-2xl border border-[var(--color-border)] p-4">
            ...
            <div className="flex items-center gap-1">
              {[
                { label: "Registered", value: patientFlow.registered, color: "bg-blue-500", textColor: "text-blue-600" },
                { label: "Waiting",    value: patientFlow.waiting,    color: "bg-amber-500", textColor: "text-amber-600" },
                { label: "Consultation",value: patientFlow.consultation,color: "bg-[var(--color-primary-600)]", textColor: "text-[var(--color-primary-600)]" },
                { label: "Completed",  value: patientFlow.completed,  color: "bg-emerald-500", textColor: "text-emerald-600" },
              ].map((step, i, arr) => (
                <div key={step.label} className="flex items-center gap-1 flex-1 min-w-0">
                  <div className="flex-1 min-w-0 text-center">
                    <div className={clsx("w-10 h-10 rounded-full flex items-center justify-center mx-auto mb-1.5", step.color)}>
                      {i === 0 && <Users size={16} className="text-white" />}
                      {i === 1 && <Clock size={16} className="text-white" />}
                      {i === 2 && <Stethoscope size={16} className="text-white" />}
                      {i === 3 && <UserCheck size={16} className="text-white" />}
                    </div>
                    <p className={clsx("text-[11px] font-semibold", step.textColor)}>{step.label}</p>
                    <p className="text-lg font-bold text-[var(--color-ink-900)] tabular-nums">{step.value}</p>
                  </div>
                  {i < arr.length - 1 && (
                    <ArrowRight size={12} className="text-[var(--color-ink-300)] shrink-0" />
                  )}
                </div>
              ))}
            </div>
                  ...
                  <div key={f.id} className="flex items-center justify-between py-2.5 gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-[var(--color-ink-400)] mt-0.5">
                        {format(new Date(f.dateTime), "dd MMM · hh:mm a")}
