...
import { ArrowLeft, UploadCloud, Camera, X, FileText, Aperture, User, CreditCard } from "lucide-react";
...
// ─── Single-slot photo/doc upload box ──────────────────────────────────────
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {camOpen && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/80 p-4">
          <div className="relative w-full max-w-lg">
            <button type="button" onClick={stopCam} className="absolute top-3 right-3 z-10 size-9 flex items-center justify-center rounded-full bg-white/20 text-white hover:bg-white/30"><X size={18} /></button>
            <video ref={videoRef} autoPlay playsInline muted className="w-full rounded-xl" />
            <div className="flex justify-center mt-4">
              <button type="button" onClick={capture} className="flex items-center gap-2 px-8 py-3 rounded-full bg-white text-[var(--color-ink-900)] font-semibold text-sm hover:bg-gray-100 shadow-lg">
                <Aperture size={18} /> Capture
              </button>
            </div>
          </div>
...
export function NewPatientForm({
  ...
  function handleSubmit(formData: FormData) {
    ...
    if (aadhaarPhoto) formData.set("aadhaarPhotoFile", aadhaarPhoto.savedName);
    if (patientPhoto) formData.set("patientPhotoFile", patientPhoto.savedName);
      ...
      <div className="w-full max-w-2xl">
        ...
        <Card lifted>
          <form action={handleSubmit} className="flex flex-col gap-5">
            ...
            {/* Photos & Documents */}
            <p className="text-xs font-semibold tracking-widest text-[var(--color-ink-400)] uppercase mt-1">Photos & Documents</p>
            <div className="grid grid-cols-2 gap-4">
              <PhotoUploadBox
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <label className="text-sm font-medium text-[var(--color-ink-700)]">Notes / Instructions (optional)</label>
                <textarea name="complaint" rows={3} placeholder="Any special instructions or referral details..." className={`${INPUT} resize-none`} />
              </div>
              <div className="col-span-2">
